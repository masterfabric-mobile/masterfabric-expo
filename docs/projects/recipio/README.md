...
| 2 | **Onboarding** | Multi-step intro (e.g. Enter Ingredients → Get Matches → Cook); Skip / Get Started; state persisted (Zustand + AsyncStorage). | 1 ✅ |
| 3 | **Home (Dashboard)** | Greeting, current plan card, **Find Your Next Meal** card (search icon in header), Cook Tonight (Supabase recipes), Recent Activity, bottom tabs. | 1 ✅ |
| 4 | **Ingredient Input** | Add ingredients (name, quantity, unit), autocomplete, category filter, “Suggest Recipes” → recipe list. | 2 |
| 5 | **Recipe List** | Recipes by ingredients or general list; compatibility %, sort/filter; from `v_public_recipe_cards`. | 2 |
| 6 | **Recipe Detail** | Hero image, back/favorite, name, rating, time/difficulty/servings, description, ingredients (available/missing), steps, **servings stepper** (1–4), Start Cooking, nutrition; data from `v_recipe_detail`. | 2 |
| 7 | **Cooking Guide** | Step-by-step mode: progress, step content, optional timer/tip, Previous/Next/Complete. | 2 |
| 8 | **Favorites** | List of favorited recipes; search/filter; remove from favorites. | 2 |
| 9 | **History** | Saved and tried recipe history. | 2 |
| 10 | **Profile** | User info, stats, settings (language, theme, notifications), sign out. | 3 |
| 11 | **Auth** | Login / Sign up (Supabase Auth); protected routes (optional). | 3 |

---

## 3. System Architecture

High-level: **Expo app** → **screens (feature-based)** → **shared services** → **Supabase**.
